...
async function loadProfile(userId = null) {
    ...
    return await apiCall(`profile${queryParam}`);
}

async function loadScraps(userId = null, page = 1) {
    const params = new URLSearchParams({ page });
    if (userId) params.append('userId', userId);
    return await apiCall(`scraps?${params}`);
}

async function sendScrap(toUserId, content) {
    return await apiCall('scraps', {
        method: 'POST',
        body: { toUserId, content }
    });
}

async function loadFriends(type = 'friends', page = 1) {
    return await apiCall(`friends?type=${type}&page=${page}`);
}
...
async function searchUsers(query, type = 'users') {
    return await apiCall(`search?q=${encodeURIComponent(query)}&type=${type}`);
}
